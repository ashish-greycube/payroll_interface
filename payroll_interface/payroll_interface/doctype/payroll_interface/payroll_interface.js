...
frappe.provide("payroll_interface.payroll_interface");
frappe.provide("payroll_interface.payroll_interface");
...
frappe.ui.form.on("Payroll Interface", {
	...
	refresh: function(frm) {
		...
		// 		return payroll_interface.payroll_interface.reverse_journal_entry(frm);
		// 	}, __('Make'));
		// }

		// if (frm.doc.__islocal) {
		// 	frm.add_custom_button(__('Quick Entry'), function() {
		// 		return payroll_interface.payroll_interface.quick_entry(frm);
		// 	});
		// }

		// hide /unhide fields based on currency
		payroll_interface.payroll_interface.toggle_fields_based_on_currency(frm);
	...
	make_inter_company_journal_entry: function(frm) {
		...
		d.set_primary_action(__('Create'), function() {
			...
			frappe.call({
				...
				method: "payroll_interface.payroll_interface.doctype.payroll_interface.payroll_interface.make_inter_company_journal_entry",
				callback: function (r) {
					if (r.message) {
	...
	posting_date: function(frm) {
		...
		$.each(frm.doc.accounts || [], function(i, row) {
			payroll_interface.payroll_interface.set_exchange_rate(frm, row.doctype, row.name);
		})
	},
...
payroll_interface.payroll_interface.PayrollInterface = frappe.ui.form.Controller.extend({
	onload: function() {
		this.load_defaults();
		this.setup_queries();
	...
	setup_queries: function() {
		...
		me.frm.set_query("account", "accounts", function(doc, cdt, cdn) {
			return payroll_interface.payroll_interface.account_query(me.frm);
		});
		...
		me.frm.set_query("reference_name", "accounts", function(doc, cdt, cdn) {
			...
			if(jvd.reference_type==="Journal Entry") {
				...
				return {
					query: "payroll_interface.payroll_interface.doctype.payroll_interface.payroll_interface.get_against_jv",
					filters: {
						account: jvd.account,
						party: jvd.party
					}
	...
	get_outstanding: function(doctype, docname, company, child, due_date) {
		...
		return frappe.call({
			method: "payroll_interface.payroll_interface.doctype.payroll_interface.payroll_interface.get_outstanding",
			args: { args: args},
			callback: function(r) {
				if(r.message) {
...
cur_frm.script_manager.make(payroll_interface.payroll_interface.PayrollInterface);
...
cur_frm.cscript.voucher_type = function(doc, cdt, cdn) {
	...
	if((!(doc.accounts || []).length) || ((doc.accounts || []).length==1 && !doc.accounts[0].account)) {
		if(in_list(["Bank Entry", "Cash Entry"], doc.voucher_type)) {
			return frappe.call({
				type: "GET",
				method: "payroll_interface.payroll_interface.doctype.payroll_interface.payroll_interface.get_default_bank_cash_account",
				args: {
			...
			return frappe.call({
				type:"GET",
				method: "payroll_interface.payroll_interface.doctype.payroll_interface.payroll_interface.get_opening_accounts",
				args: {
					"company": doc.company
				},
				callback: function(r) {
...
frappe.ui.form.on("Payroll Interface Account", {
	party: function(frm, cdt, cdn) {
		...
		if(!d.account && d.party_type && d.party) {
			...
			return frm.call({
				method: "payroll_interface.payroll_interface.doctype.payroll_interface.payroll_interface.get_party_account_and_balance",
				child: d,
				args: {
					company: frm.doc.company,
	...
	cost_center: function(frm, dt, dn) {
		payroll_interface.payroll_interface.set_account_balance(frm, dt, dn);
	},
	...
	debit_in_account_currency: function(frm, cdt, cdn) {
		payroll_interface.payroll_interface.set_exchange_rate(frm, cdt, cdn);
	},
	...
	exchange_rate: function(frm, cdt, cdn) {
		...
		payroll_interface.payroll_interface.set_debit_credit_in_company_currency(frm, cdt, cdn);
	}
})
...
$.extend(payroll_interface.payroll_interface, {
	...
	set_exchange_rate: function(frm, cdt, cdn) {
		...
		if(row.account_currency == company_currency || !frm.doc.multi_currency) {
			...
			frappe.call({
				method: "payroll_interface.payroll_interface.doctype.payroll_interface.payroll_interface.get_exchange_rate",
				args: {
		...
		var dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{fieldtype: "Link", fieldname: "debit_account", label: __("Debit Account"), reqd: 1,
					options: "Account",
					get_query: function() {
						return payroll_interface.payroll_interface.account_query(frm);
					}
				},
...
$.extend(payroll_interface.payroll_interface, {
	set_account_balance: function(frm, dt, dn) {
		...
		if(d.account) {
			...
			return frappe.call({
				method: "payroll_interface.payroll_interface.doctype.payroll_interface.payroll_interface.get_account_balance_and_party_type",
				args: {
					account: d.account,
				...
				callback: function(r) {
					if(r.message) {
						$.extend(d, r.message);
						payroll_interface.payroll_interface.set_debit_credit_in_company_currency(frm, dt, dn);
						refresh_field('accounts');
					}
				}
			});
